...
static void demoWithoutLock() {
    std::cout << "\n  ╔══════════════════════════════════════════════════╗\n";
    std::cout << "  ║  KHONG CO MUTEX - Hai dau cuoi cung luc rut     ║\n";
    std::cout << "  ╚══════════════════════════════════════════════════╝\n";
    ...
    if (acct.balance < 0.0) {
        std::cout << "  <-- AM! CHI TIEU KEP THANH CONG (loi bao mat!)";
    }
    std::cout << "\n";
}
...
static void demoWithLock(Bank& bank) {
    std::cout << "\n  ╔══════════════════════════════════════════════════╗\n";
    std::cout << "  ║  CO MUTEX - He thong Apex khoa tai khoan         ║\n";
    std::cout << "  ╚══════════════════════════════════════════════════╝\n";
    ...
    auto tryWithdraw = [&](const std::string& name) {
        ...
        try {
            ...
            ++successCount;
            std::lock_guard<std::mutex> pl(printMtx);
            std::cout << "    [" << name << "] >>> RUT THANH CONG!\n";
        } catch (const ApexError& e) {
            std::lock_guard<std::mutex> pl(printMtx);
            std::cout << "    [" << name << "] XXX Bi tu choi: " << e.what() << "\n";
        }
    };
    ...
    if (successCount.load() <= 1) {
        std::cout << "  [OK] Mutex bao ve thanh cong! Chi 1 giao dich duoc chap thuan.\n";
    }
}
...
void runInvalidRate(Bank& bank) {
    ...
    for (long double rate : badRates) {
        ...
        try {
            ...
            std::cout << "  [!] Khong nen den day — ty gia da duoc dat sai!\n";
        } catch (const InvalidRate& e) {
            std::cout << "  [OK] He thong tu choi: " << e.what() << "\n";
        }
    }
